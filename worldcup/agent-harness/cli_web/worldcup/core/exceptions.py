"""Typed exception hierarchy for cli-web-worldcup.
...
class WorldcupError(Exception):
    """Base exception for all cli-web-worldcup errors."""
...
class RateLimitError(WorldcupError):
...
class NetworkError(WorldcupError):
    """Connection failed -- DNS resolution, TCP connect, TLS handshake."""


class ServerError(WorldcupError):
...
def _error_code_for(exc: WorldcupError) -> str:
